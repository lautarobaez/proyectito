...
"""
lan_monitor_gui.py
Aplicación de escritorio para monitoreo LAN usando solo Python stdlib (Tkinter + sqlite3).
- Escanea la subred por ping
- Obtiene MAC consultando 'arp -a' o 'ip neigh'
- Resuelve hostname con socket
- Guarda en SQLite y muestra en GUI con Tkinter
    ...
    def export_csv(self):
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8") as f:
                cw = csv.writer(f)
                cw.writerow(["ip","mac","hostname","last_seen"])
                cw.writerows(rows)
